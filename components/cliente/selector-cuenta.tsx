  ...
  return (
    <div ref={raiz} className="relative min-w-0" onKeyDown={teclas}>
      <button type="button" aria-haspopup="listbox" aria-expanded={abierto} aria-label={`Cuenta publicitaria: ${actual.nombre}. Cambiar`} onClick={() => setAbierto((v) => !v)} className={`group flex items-center gap-2.5 rounded-full bg-marino py-1 pl-1.5 pr-3 text-left text-white transition-[box-shadow,transform] duration-300 hover:shadow-[0_12px_24px_-12px_rgba(11,29,58,0.6)] ${cambiando ? 'opacity-70' : ''}`}>
        <span aria-hidden="true" className="grid h-7 w-7 place-items-center rounded-full bg-white/12 text-[10px] font-bold text-celeste">{actual.nombre.split(' ').map((p: string) => p[0]).slice(0, 2).join('')}</span>
      ...
      {abierto && (
        <ul role="listbox" aria-label="Cuentas publicitarias" className="absolute left-0 top-[calc(100%+8px)] z-50 min-w-[300px] overflow-hidden rounded-[16px] bg-superficie p-1.5 shadow-[0_24px_50px_-20px_rgba(11,29,58,0.5)] ring-1 ring-borde entra-zoom">
          {cuentas.map((c, i) => {
            ...
            return (
              <li key={c.id} role="option" aria-selected={activa} onMouseEnter={() => setFoco(i)} onClick={() => elegir(c)} className={`flex cursor-pointer items-center gap-2.5 rounded-[11px] px-2.5 py-2 transition-colors ${i === foco ? 'bg-superficie-2' : ''} ${activa ? 'text-texto' : 'text-texto-2'}`}>
                <span aria-hidden="true" className={`grid h-7 w-7 shrink-0 place-items-center rounded-full text-[10px] font-bold ${activa ? 'bg-marino text-celeste' : 'bg-superficie-2 text-cobalto'}`}>{c.nombre.split(' ').map((p: string) => p[0]).slice(0, 2).join('')}</span>
