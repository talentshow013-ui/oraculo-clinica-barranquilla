...
function resolverCuentas(lote: LoteDatos, cfg: ConfigCliente, fuentes: EstadoFuente[]): CuentaPublicitaria[] {
  ...
  const extra = [...conPauta].filter((id) => !cfg.cuentasPublicitarias.some((c) => c.id === id)).map<CuentaPublicitaria>((id) => ({ id, nombre: id.startsWith("tt_") ? `TikTok · ${id.slice(3)}` : id.startsWith("ga_") ? `Google Ads · ${id.slice(3)}` : id, plataforma: id.startsWith("tt_") ? "tiktok" : id.startsWith("ga_") ? "google" : "meta", moneda: "COP", activa: true, ultimaSincronizacion: ultima }));
