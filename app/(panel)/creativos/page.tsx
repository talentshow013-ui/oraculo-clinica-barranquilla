...
export default async function Creativos() {
  ...
  // Número corto por inversión (1 = el que más gastó): es lo que lleva la burbuja y la columna #.
  const numeros = new Map([...r.creativos].sort((a, b) => b.agregado.gasto - a.agregado.gasto).map((c, i) => [c.creativo.id, i + 1] as const))
  const filas = r.creativos.map((c) => ({
    clave: c.creativo.id,
    crudo: { id: numeros.get(c.creativo.id) ?? 0, titular: etiquetaCreativo(c.creativo), cuadrante: c.cuadrante, hook: c.hookRate, hold: c.holdRate, ctr: c.ctrEnlace, costo: c.costoResultado, gasto: c.agregado.gasto, fatiga: c.fatiga.indice, vida: c.vidaUtilDias, dias: c.diasActivo },
    celdas: {
      id: <span className="num font-semibold">{numeros.get(c.creativo.id)}</span>,
      titular: <span className="block max-w-[300px]"><span className="block truncate font-medium" title={etiquetaCreativo(c.creativo, 200)}>{etiquetaCreativo(c.creativo)}</span><span className="block truncate text-[11.5px] text-texto-2">{c.creativo.formato} · {ANGULOS[c.creativo.anguloDetectado]} · nivel {c.creativo.nivelConsciencia}</span></span>,
  ...
  return (
    <>
      ...
      <div className="grid grid-cols-1 gap-3 lg:grid-cols-12">
        <Panel rotulo="Gancho × costo por resultado" titulo="La matriz: cada burbuja es un anuncio, su tamaño es la inversión" className="lg:col-span-8"><Cuadrantes creativos={r.creativos} numeros={numeros} /></Panel>
        <div className="flex flex-col gap-3 lg:col-span-4">
          <Panel tono="marina" rotulo="Fatiga · fórmula visible" titulo="Cómo se calcula">
            ...
            {r.creativos.filter((c) => (c.fatiga.indice ?? 0) >= 0.6).map((c) => <p key={c.creativo.id} className="mt-2 rounded-[12px] bg-mal/20 px-3 py-2 text-[13px] text-white ring-1 ring-mal/40">«{etiquetaCreativo(c.creativo, 40)}» va en {indice(c.fatiga.indice)}: le quedan ~{num(c.vidaUtilDias)} días. Ten el siguiente listo.</p>)}
          </Panel>
